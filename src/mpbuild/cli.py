...
from .build import build_board, clean_board, make_command
...
def make_command(
    extra_args: Annotated[
        Optional[List[str]], typer.Argument(help="arguments to pass to make")
    ] = None,
    port: Annotated[
        Optional[str], 
        typer.Option("--port", "-p", help="Force specific port", autocompletion=_complete_port)
    ] = None,
) -> None:
    """
    Run make in the appropriate container based on the current directory or specified port.
    """
    make_command(extra_args, port)
